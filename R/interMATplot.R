...
#' The x and y arguments refer to x-axis and y-axis.
...
#' @param tad.chr_y,tad.chr_x Chromosome name to filter annotations (domains and loop). Default is `NULL`
...
interMATplot <- function(matrix, bin.width, start_y = NULL, stop_y = NULL, start_x = NULL, stop_x = NULL, log2 = T, scale.colors = "H", scale.limits = NULL,
  ...
  if (!is.null(tad.x.line)) {
    ...
    p = p + ggplot2::geom_segment(data = tad, ggplot2::aes(x = s2, y = -min_graph1, xend = e2, yend = -min_graph1, col = col), linewidth = 1.5)+
      ggplot2::geom_segment(data = tad, ggplot2::aes(x = s2, y = -max_graph1, xend = e2, yend = -max_graph1, col = col), linewidth = 1.5)
  }
  ...
  if (!is.null(loop.bedpe)) {
    ...
    p = p +
      ggplot2::geom_rect(data = loop, ggplot2::aes(xmin = s_x, xmax = e_x, ymin = -s_y, ymax = -e_y),
                         fill = NA, color = annotations.color, linewidth = 0.3)
  }
